...
#define   MESH_SIZE                   4
#define   NODE_SOURCE                 "30"
#define   INDOOR_ANEMOMETER_SOURCE    "31"
#define   OUTDOOR_ANEMOMETER_SOURCE   "32"
...
String outdoorURL = "https://api.weather.com/v2/pws/observations/current?stationId=IQUEZO23&format=json&units=m&apiKey=9d4f41efcb5647a58f41efcb56d7a5d3&numericPrecision=decimal";
String indoorURL = "https://api.weather.com/v2/pws/observations/current?stationId=IQUEZO24&format=json&units=m&apiKey=9d4f41efcb5647a58f41efcb56d7a5d3&numericPrecision=decimal";
...
Task taskPublishSensorData(TASK_SECOND * 10, TASK_FOREVER, &publishSensorData);
Task taskPublishOutdoorAnemometerData(TASK_SECOND * 10, TASK_FOREVER, &publishOutdoorAnemometerData);
Task taskPublishIndoorAnemometerData(TASK_SECOND * 10, TASK_FOREVER, &publishIndoorAnemometerData);
...
void sdCreateFile(const char* fileName, String dataType) {
  ...
  if(!file) {
    ...
    else if (dataType == "Anemometer") {
      Serial.println("Creating anemometer log file...");
      writeFile(SD, fileName, "Source, Time, Wind Speed (m/s), Wind Gust (m/s), Wind Direction (°) \r\n");
    }
    else if (dataType == "SensorDelay") {
      Serial.println("Creating Sensor delay log file...");
      writeFile(SD, fileName, "Sensor Data Publish Delay Time \r\n");
    }
    else {
      Serial.println("Creating Anemometer delay log file...");
      writeFile(SD, fileName, "Anemometer Data Publish Delay Time \r\n");
    }
  }
  else {
...
void sdInit() {
  ...
  sdCreateFile("/SensorPublishDelay.txt", "SensorDelay");
  sdCreateFile("/AnemometerPublishDelay.txt", "AnemometerDelay");
}
...
void publishMessage(const char* topic, String payload , boolean retained, String dataType){
  ...
  if (client.publish(topic, payload.c_str(), true)) {
    ...
    if(dataType == "Sensor") {
      ...
      appendFile(SD, "/SensorPublishDelay.txt", String(endTime - startTime).c_str());
    } else if(dataType == "Outdoor Anemometer") {
      sdOutdoorAnemometerLog("/OutdoorAnemometerLogPublished.txt");
      appendFile(SD, "/AnemometerPublishDelay.txt", String(endTime - startTime).c_str());
    } else {
